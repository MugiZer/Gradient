...
export default function CodexTranscript({ items, busy }: { items: TranscriptItem[]; busy: boolean }) {
  ...
  useEffect(() => {
    if (follow.current && view.current) view.current.scrollTop = view.current.scrollHeight;
  }, [items, busy]);
  ...
  >
    ...
    <div className="transcript-content">
      {items.map((item) => item.kind === 'user'
        ? <UserTurn key={item.id} text={item.text} />
        : item.kind === 'agent'
          ? <AgentTurn key={item.id} text={item.text} />
          : item.kind === 'diff'
            ? <DiffPreview key={item.id} text={item.text} />
            : <ToolAction key={item.id} text={item.text} status={item.status} />)}
