# Gradient client design system

Source of truth: ../DESIGN.md, sections 60–84. The user's September 12 avatar references refine the character language: simple silhouettes, expressive paired eyes, controlled role variation, and one root that deploys two agents.

ENERGY 1 / RHYTHM 1 / MOTION 2. The conversation is the primary surface. Gradient is one peripheral object, with no sidebar, dashboard, pipeline graph, or permanent agent tabs.
...
| Typography | system sans for controls, Cascadia Code / SFMono-Regular / Consolas for conversation and code | Preserve familiar desktop controls and a readable Codex transcript without font downloads. |
| Spacing | 4, 8, 12, 16, 24 px | Use one spacing scale for compact surfaces. |
| Radii | 8, 12, 16 px | Distinguish controls, lesson surfaces, and the composer. |
...
`LayoutGroup`, the `gradient-surface` layout ID, and `AnimatePresence` connect the root, tray, and artifacts. Exits reverse toward the same corner. Transform and opacity carry the motion. The transcript does not animate per token.
...
Run `npm run dev`, then open `/dev/ui`. The route labels its data as illustrative and never connects to the backend. It exists only in Vite development mode. All nine stage buttons are manual; there is no timed simulation.

Screenshots and the recorded control checks are listed in `VERIFICATION.md`.
