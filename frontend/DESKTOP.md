...
Right-click the sprite or use its tray menu:

- **Conversation** observes the connected Codex task. **Teach lesson** runs the Observer.
- **Training** reads the backend-owned `runs/demo/current.json` every two seconds. It displays the recorded PRE score and actual training/POST status. Valid real POST artifacts become available as proof automatically. These eight tasks are training tasks; this view makes no held-out transfer claim.
- **Lesson history** works without FastAPI. Click **Lesson found**, **Teach lesson**, then use **Continue lesson** for Observer and Builder. Click **Post-train agent**, **Continue lesson**, and **Run proof**. The reference label and proof identify hand-authored solutions separately from trained model results. Scores display pass rates as percentages.
...
The compact **Lesson source** selector switches between **Live lesson**, **Saved lesson**, and **Training**. The saved view uses the same lesson components and reads the recorded capability and task specifications; reference results remain labeled. Details show the Observer objective, Builder task prompts and splits, and an actual Prime run link. Raw events stay inside a collapsed event log.
